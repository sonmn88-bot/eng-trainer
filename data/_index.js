// ── 단어 세트 카탈로그 ──────────────────────────
// 새 단어장 추가 시 여기에만 항목 추가하면 됨
const WORD_CATALOG = [
  {
    id:       'personal',
    name:     '전체본',
    subname:  '선생님 직접 수집',
    icon:     '⭐',
    file:     'personal.js',
    varName:  'PERSONAL',
    color:    '#C9A961',
    tag:      'CUSTOM',
    tagColor: '#C9A961',
    desc:     '직접 모은 핵심 단어 모음',
  },
  {
    id:       'core',
    name:     '수능 핵심 필수',
    subname:  'AI + 교육부 선정',
    icon:     '🎯',
    file:     'core-essential.js',
    varName:  'WM_CORE',
    color:    '#F87171',
    tag:      'AI PICK',
    tagColor: '#F87171',
    desc:     '수능 빈출 핵심어 + 교육부 선정 2,284개',
    totalWords: 2284,
    wordsPerWeek: 150,
  },
  {
    id:       'wm-basic',
    name:     'WM 고등 Basic',
    subname:  'Word Master',
    icon:     '📗',
    file:     'wm-basic.js',
    varName:  'WM_BASIC',
    color:    '#4ADE80',
    tag:      'BASIC',
    tagColor: '#4ADE80',
    desc:     '고등 기본 필수 어휘',
  },
  {
    id:       'wm-hyper',
    name:     'WM Hyper 1000',
    subname:  'Word Master',
    icon:     '📘',
    file:     'wm-hyper1000.js',
    varName:  'WM_HYPER',
    color:    '#60A5FA',
    tag:      'ADVANCED',
    tagColor: '#60A5FA',
    desc:     '고난도 심화 어휘 1000',
  },
  {
    id:       'wm-suneung',
    name:     'WM 수능 2000',
    subname:  'Word Master',
    icon:     '📙',
    file:     'wm-suneung2000.js',
    varName:  'WM_SUNEUNG',
    color:    '#FBBF24',
    tag:      'SUNEUNG',
    tagColor: '#FBBF24',
    desc:     '수능 빈출 2000 완전정복',
  },
  // ── 여기에 새 단어장 추가 ──
  // {
  //   id: 'wm-xxx', name: '워드마스터 XXX', subname: 'Word Master',
  //   icon: '📕', file: 'wm-xxx.js', varName: 'WM_XXX',
  //   color: '#A78BFA', tag: 'NEW', tagColor: '#A78BFA', desc: '설명',
  // },
  {
    id:       'wm-edu',
    name:     '교육부 기본어휘',
    subname:  '교육부 공식 선정',
    icon:     '📚',
    file:     'wm-edu.js',
    varName:  'WM_EDU',
    color:    '#34D399',
    tag:      '교육부',
    desc:     '교육부 선정 필수 기본어휘 2,974개',
    totalWords: 2974,
  },
  {
    id:       'ebs-intro',
    name:     'EBS 중학 입문',
    subname:  'MY VOCA COACH',
    icon:     '🌱',
    file:     'ebs-voca-intro.js',
    varName:  'EBS_VOCA_INTRO',
    color:    '#60A5FA',
    tag:      '입문',
    tagColor: '#60A5FA',
    desc:     '기초가 약한 학생을 위한 중학 입문 단계 960개',
    totalWords: 960,
    wordsPerWeek: 150,
  },
  {
    id:       'ebs-basic',
    name:     'EBS 중학 기본',
    subname:  'MY VOCA COACH',
    icon:     '📘',
    file:     'ebs-voca-basic.js',
    varName:  'EBS_VOCA_BASIC',
    color:    '#34D399',
    tag:      '기본',
    tagColor: '#34D399',
    desc:     '중학 기본 단계 어휘 1,170개',
    totalWords: 1170,
    wordsPerWeek: 150,
  },
  {
    id:       'ebs-adv',
    name:     'EBS 중학 실력',
    subname:  'MY VOCA COACH',
    icon:     '📕',
    file:     'ebs-voca-adv.js',
    varName:  'EBS_VOCA_ADV',
    color:    '#F472B6',
    tag:      '실력',
    tagColor: '#F472B6',
    desc:     '중학 실력 완성 단계 어휘 1,176개',
    totalWords: 1176,
    wordsPerWeek: 150,
  },
];
window.WORD_CATALOG = WORD_CATALOG; // teacher.html 등 window 참조용

// ── 리스닝 세트 카탈로그 ─────────────────────────
// 새 회차 추가 시 여기에만 항목 추가
const LISTENING_CATALOG = [
  {
    id:       '2026-suneung',
    name:     '2026학년도 수능',
    subname:  '대학수학능력시험',
    icon:     '🎯',
    metaFile: '2026-suneung-meta.json',
    audioDir: '2026-suneung',
    color:    '#C9A961',
    tag:      '수능',
    tagColor: '#C9A961',
    total:    17,
  },
  {
    id:       '2026-09',
    name:     '2026학년도 9월 모의',
    subname:  '전국연합학력평가',
    icon:     '📝',
    metaFile: '2026-09-meta.json',
    audioDir: '2026-09',
    color:    '#60A5FA',
    tag:      '9모',
    tagColor: '#60A5FA',
    total:    17,
  },
  {
    id:       '2026-06',
    name:     '2026학년도 6월 모의',
    subname:  '전국연합학력평가',
    icon:     '📋',
    metaFile: '2026-06-meta.json',
    audioDir: '2026-06',
    color:    '#A78BFA',
    tag:      '6모',
    tagColor: '#A78BFA',
    total:    17,
  },
  // ── 여기에 새 회차 추가 ──
];
window.LISTENING_CATALOG = LISTENING_CATALOG;
